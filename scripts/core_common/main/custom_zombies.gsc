...
StartCountdown()
{
    ...
    wait 2;
    ShieldHudElemSetText(#"Countdown", "");
    thread StartRound();
}
...
StartRound()
{
    ...
    if (Blackout() && level.currentRound % 6 == 0)
    {
        level.dogRoundCount++;
        foreach (player in level.players)
        {
            if(!player isHost()) player iprintlnbold("^1Hellhounds incoming! Survive the round!");
        }
...
MonitorZombie(Zombie, player)
{
    ...
    while (isDefined(Zombie) && isDefined(player))
    {
        wait 1;
