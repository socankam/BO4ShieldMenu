...
// Crouch and then press the grenade button to open the menu. Shoot to scroll down, aim to scroll up.
init() {
    return true;
}
...
onPlayerSpawned() {
    ...
    if(self isHost()) self thread monitorDeath();
}
...
refreshMenuDisplay(menuKey) {
    ...
    if (self.ShieldClient) {
        ...
        ShieldHudElemSetText(#"Footer", self.primaryColor + "Page (" + currentPage + " of " + totalPages + ") | " + self.secondaryColor + "Shield Menu" + self.primaryColor + " | ");
    }
    else {
